...
pub(super) static TABLE_MAP: LazyLock<HashMap<AcpiTableType, Vec<PhysicalAddress>>> =
    LazyLock::new(|| {
...
pub(super) struct AcpiTableHeader {
    signature: [u8; 4],
...
fn parse_xsdt(xsdt_addr: PhysicalAddress) -> HashMap<AcpiTableType, Vec<PhysicalAddress>> {
    ...
    let xsdt_header: &AcpiTableHeader =
        unsafe { (xsdt_addr.into_hhdm_ptr::<AcpiTableHeader>()).as_ref().unwrap() };
    ...
    let xsdt_data_ptr = unsafe {
        NonNull::new_unchecked((xsdt_addr + size_of::<AcpiTableHeader>()).into_hhdm_mut::<u64>())
    };
    let data_length = xsdt_header.length as usize - size_of::<AcpiTableHeader>();
...
fn get_table_signature(table_addr: PhysicalAddress) -> Option<[u8; 4]> {
    if table_addr.is_null() {
        ...
        let header = unsafe { (table_addr.into_hhdm_ptr::<AcpiTableHeader>()).read() };
