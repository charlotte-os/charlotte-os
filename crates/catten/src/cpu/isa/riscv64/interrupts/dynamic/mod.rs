//! # RISC-V Dynamic Interrupt Dispatch
//!
//! The matrix of per-hart handlers that external interrupts are dispatched through once the
//! PLIC/APLIC driver can route them.
...
impl DynIhMapIfce for DynIhMap {
    fn set_dyn_ih(
        ...
        todo!("Record the handler once RISC-V external interrupt vectors are allocated per hart.")
    }

    extern "C" fn get_local_dyn_ih(&self, _vector: IntSrcDscr) -> Option<InterruptHandler> {
        todo!("Look the handler up from the trap entry on the current hart.")
    }

    fn clear_dyn_ih(&mut self, _lp: LpId, _vector: IntSrcDscr) -> Result<(), Error> {
        todo!("Release a previously assigned RISC-V interrupt vector.")
    }

    fn find_available_target(&self) -> Option<InterruptTarget> {
        todo!("Pick a hart and a free vector for a newly routed external interrupt.")
    }
}
